module Youtube
  class VideoImporter
    ...
    def import_video(yt_video)
      ...
      return unless yt_video.views[:total] && yt_video.views[:total] >= 1000
      ...
      ViewStat.transaction do
        import_baseline_for_video(all_view_stats, video, yt_video)

        # Import separately per day for the last week
        (1.week.ago.to_date...Date.today).each do |day|
          import_video_stats_for_day(day, all_view_stats, video, yt_video)
        end
      end
    ...
    def import_video_stats_for_day(day, all_view_stats, video, yt_video)
      ...
      day_views_in_all_countries = Hash[yt_video.viewer_percentage(since: day, until: day).map do |gender, dist|
                                          [gender, Hash[dist.map do |age_group, percentage|
                                                          [age_group, (((percentage || 0) * (yt_video.views(since: day, until: day)[:total] || 0)) / 100).round]
                                                        end]]
                                        end]
      [:male, :female].product(%w(65- 35-44 45-54 13-17 25-34 55-64 18-24)).each do |gender, age_group|
        next unless day_views_in_all_countries && day_views_in_all_countries[gender]
        ...
        view_stat.number_of_views = (day_views_in_all_countries[gender][age_group] || 0) - day_views_in_countries_we_care_about[gender][age_group]
        all_view_stats << view_stat
      end
    end

    def import_baseline_for_video(all_view_stats, video, yt_video)
